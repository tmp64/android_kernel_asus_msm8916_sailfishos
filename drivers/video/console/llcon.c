 ...
 *
 * ARM64 fixes:
 * @author	tmp64 <tmp32x2@gmail.com>
 * @date	2019-01-19
 */
...
/*
* androidboot.llcon=<mode>,<delay>,<textwrap>,<fb_bpp>,<fb_height>,
*                   <fb_width>,<fb_stride>,<font_size>,<font_color>
* llcon_fb_addr=<fb_addr>
*/
...
static int __init llcon_setup(char *str)
{
	...
	case 8:
		font_size = ints[8];
	case 7:
		llcon.display.stride = ints[7];
	case 6:
		llcon.display.width = ints[6];
	case 5:
		llcon.display.height = ints[5];
	case 4:
		llcon.display.bpp = ints[4];
	case 3:
...
static int __init llcon_fb_addr(char *buf)
{
	unsigned long addr;
	int rc;
	rc = kstrtoul(buf, 0, &addr);
	if (rc)
	{
		pr_err("LLCON: invalid llcon_fb_addr ('%s')\n", buf);
		return 0;
	}
	llcon.display.bl_fbaddr = (void *)addr;
	return 1;
}
early_param("llcon_fb_addr", llcon_fb_addr);
...
static int __init llcondmp_setup(char *str)
{
	...
	case 2:
		size = (size_t)ints[2];
	case 1:
		addr = (void *)(unsigned long long)*((unsigned int *)&ints[1]);
	}
	if (size && addr)
...
void llcon_clear(void)
{
	...
	memset_io(llcon.display.fbaddr, 0, size);
}
...
int llcon_init(void)
{
	...
	if (llcon.mode == LLCON_MODE_ASYNC) {
		...
		if (IS_ERR(llcon.task)) {
			pr_err("LLCON: can't create kthread (err = %p) \n", llcon.task);
			rc = -10;
			goto err;
		}
	}
	...
	pr_info("LLCON: mode: %d, addr: %p (%p), size: %ux%u, stride: %u,"
		...
		(unsigned int)llcon.display.width, (unsigned int)llcon.display.height, (unsigned int)llcon.display.stride,
		llcon.font.width, llcon.font.height, llcon.fg_color, (unsigned int)llcon.delay);
